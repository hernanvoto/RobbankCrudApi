...
public class EntityNotFoundException extends RuntimeException {
     ...
     * 
     */
    private static final long serialVersionUID = -7305853924451507615L;
    ...
    private static <K, V> Map<K, V> toMap(Class<K> keyType, Class<V> valueType, String... entries) {
        ...
        return IntStream.range(0, entries.length / 2).map(i -> i * 2).collect(HashMap::new, (m, i) -> m.put(keyType
                .cast(entries[i]), valueType.cast(entries[i + 1])), Map::putAll);
    }
